...
wishlistRouter.post("/post/:productID", UserAuth, async (req, res) => {
  ...
  try {
    let isProduct = await wishlistRouter.findOne({ productID, userID });
    if (isProduct) {
      res.status(401).send({ message: "This Product Is Already In Wishlist!" });
    } else {
      await wishlistRouter.create({ productID, userID });
      res.send({ message: "Product Added To The Wishlist Successfully" });
    }
  } catch (error) {
    res
      .status(401)
